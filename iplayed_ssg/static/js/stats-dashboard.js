(function () {
    ...
    function initStatsDashboard() {
        ...
        const completionsUrl = statsRoot.dataset.completionsUrl;
        const gamesBaseUrl = statsRoot.dataset.gamesBaseUrl || '';
        ...
        const formatHours = (value) => `${numberFormatter.format(value)} h`;

        function buildCompletionStripes(count, maxCount) {
            const normalizedMax = Math.max(1, maxCount);
            const filledStripes = count
                ? Math.min(
                    heatmapStripeConfig.stripeCount,
                    Math.max(1, Math.ceil((count / normalizedMax) * heatmapStripeConfig.stripeCount))
                )
                : 0;

            return Array.from({ length: heatmapStripeConfig.stripeCount }, (_, index) => {
                const isFilled = index < filledStripes;
                const symbol = isFilled ? heatmapStripeConfig.filledSymbol : heatmapStripeConfig.emptySymbol;
                const modifier = isFilled ? ' stats-heatmap__stripe--filled' : '';
                return `<span class="stats-heatmap__stripe${modifier}">${symbol}</span>`;
            }).join('');
        }
    ...
    `;
        ...
        function renderHeatmap(year, months) {
            ...
            monthIndexes.forEach((index) => {
                ...
                const stripes = buildCompletionStripes(value, maxCount);
                cell.innerHTML = `
        <span class="stats-heatmap__month">${monthNames[index]}</span>
        <span class="stats-heatmap__value">
          <span class="stats-heatmap__stripes" aria-hidden="true">${stripes}</span>
          <span class="sr-only">${value} completions</span>
        </span>
      `;
        ...
        function renderGenreComparison(genreCounts, genreHours) {
            if (!els.genreCanvas || typeof Chart === 'undefined') {
                return;
            }
        ...
        async function loadStats() {
            try {
                const response = await fetch(completionsUrl, { cache: 'no-cache' });
                if (!response.ok) {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initStatsDashboard, { once: true });
    } else {
        initStatsDashboard();
    }
})();
